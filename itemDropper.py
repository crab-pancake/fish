...
def load(player):
    ...
    for a in range(0,len(reader)):
        ...
        if level >= int(univ.Items[itemnamecode].minlvl): #Adds 'item' to the pool if the player's level is greater than 'min_lvl' Found in allitems_m.csv)
    ...
    if collect == 'x' or collect == 0:
        ...
        while collect>0:
            ...
            chosen_item_name = univ.Items[chosen_item].name
