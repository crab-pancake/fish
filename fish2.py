...
def new_acct():
  ...
  try:
    ...
    with open(uname+'.csv', 'w') as playerfile:#+'_g_info.csv'
      writer = csv.writer(playerfile, dialect = 'excel')
      writer.writerow(['Create Time', time.time()])
      writer.writerow(['Last Login', 0]) #changed lastlogin to set to 0 when 
      while True:
        pw = rinput("Enter a password longer than 3 characters, or type 'back' to cancel.\n>>")
        if pw.lower() == 'back':
          acct_ask()
          break
        elif len(pw) < 3:
