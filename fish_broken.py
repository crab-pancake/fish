...
def new_acct():
  ...
  try:
    ...
    inv= {"mackerel": 0, "cockle": 0, "s_karp": 0,"create_time": time.time(), "last_login": time.time()}##    Added create_time and last_login as members of the inventory
    with open(uname+'.csv', 'wb') as playerfile:
      writer = csv.writer(playerfile, dialect = 'excel')
      for key, value in inv.items():##
        writer.writerow([key, value])##
##  CODE TO OPEN CSV FILES FOR SAVING      
